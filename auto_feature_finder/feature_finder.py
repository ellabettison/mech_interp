...
class FeatureFinder:
    def __init__(self, model_name: str = "deepseek-r1-distill-llama-8b", sourceset: str = "15-llamascope-slimpj-res-32k",
        ...
        self.basic_dataset = datasets.load_dataset("JailbreakBench/JBB-Behaviors", "judge_comparison")["test"]
        self.real_dataset = datasets.load_dataset("JailbreakV-28K/JailBreakV-28k", "JailBreakV_28K")["mini_JailBreakV_28K"]
        logger.info("Loaded dataset with %d, %d items", len(self.basic_dataset), len(self.real_dataset))
        ...
        self.jailbreak_prompts = list(self.real_dataset["jailbreak_query"][:n_dataset_to_use]) #list(self.basic_dataset["prompt"][:n_dataset_to_use]) #+ list(self.real_dataset["jailbreak_query"][:])
    ...
    def get_feature_activation_for_text(self, prompt: str, feature_index: str) -> dict:
        ...
        if key in self.activation_cache:
            ...
            logger.info(f"Using cached activation for feature {feature_index} and prompt: {cached_activation}" )
            return cached_activation

        max_activation = self.neuronpedia.get_feature_activation_for_text_by_token(prompt, self.sourceset_to_test, feature_index)
    ...
    def compute_feature_differences(self, positive_examples, negative_examples, p_value_threshold=0.05):
        logger.info("Computing feature activation differences (statistical significance)")
        ...
        for feature_index in common_features:
            ...
            if len(pos_vals) < 2 or len(neg_vals) < 2:
                logger.debug("Skipping feature %s due to insufficient data", feature_index)
                continue
            ...
            logger.info(
                ...
                feature_index, neg_mean, pos_mean, diff, p_value
            )
    ...
    def compute_feature_differences_welch(self, positive_examples, negative_examples, min_samples=5, alpha=0.05):
        ...
        for feature_index in common_features:
            ...
            # Cohen's d
            pooled_std = np.sqrt((np.std(pos_vals, ddof=1) ** 2 + np.std(neg_vals, ddof=1) ** 2) / 2)
        ...
        differences.sort(key=lambda x: abs(x["cohen_d"]) / (x["p_value"] + 1e-8), reverse=True)
    ...
    def compute_feature_differences_welch_cached(self, p_value_threshold=0.05):
        logger.info("Computing feature differences using Welch’s t-test and cached activations")
        ...
        for key, value in self.activation_cache.items():
            try:
                ...
                logger.warning("Error parsing activation cache entry %s: %s", key, str(e))
                continue
    
        common_features = set(pos_activations.keys()) & set(neg_activations.keys())
        logger.info("Found %d common features in cached activations", len(common_features))
        ...
        for feature_index in common_features:
            ...
            t_stat, p_value = ttest_ind(neg_vals, pos_vals, equal_var=False)  # Welch's t-test
            pos_mean = np.mean(pos_vals)
            neg_mean = np.mean(neg_vals)
            diff = neg_mean - pos_mean
            ...
            logger.info(
                ...
                feature_index, pos_mean, neg_mean, diff, p_value
            )
    
            if p_value < p_value_threshold:
                results.append({"featureIndex":feature_index, "diff":diff, "p_value":p_value})
    ...
    def compute_feature_differences_with_thresholds(self, positive_examples, negative_examples, p_value_threshold=0.05, epsilon=0.2, min_neg_mean=0.5):
        logger.info("Computing feature activation differences with near-zero positive constraint")
        ...
        for feature_index in common_features:
            ...
            if pos_mean < epsilon and neg_mean > min_neg_mean and p_value < p_value_threshold:
                logger.info(
                    "Feature %s: pos_mean=%.4f, neg_mean=%.4f, diff=%.4f, p=%.4g",
                    feature_index, pos_mean, neg_mean, diff, p_value
                )
                results.append({"featureIndex":feature_index, "diff":diff, "p_value":p_value, "pos_mean":pos_mean, "neg_mean":neg_mean})
    ...
    def find_features(self):
        ...
        filtered_by_frequency = [feature for feature in featureids_to_check.items() if feature[1]>=self.min_feature_frequency]
        logger.info(f"Filtering by frequency filtered from {len(featureids_to_check)} to {len(filtered_by_frequency)}")
        filtered_featureids_to_check = sorted(filtered_by_frequency, key=lambda x: x[1], reverse=True)[:self.num_featureids_to_check]
        logger.info(f"{self.num_featureids_to_check}th most frequent feature was found {filtered_featureids_to_check[-1][1]} times")
        ...
        diffs = self.compute_feature_differences_with_thresholds(comply_features, refused_features)
        ...
        for info in diffs:
            ...
            feature_info = info
            try:
                feature_info.update(self.neuronpedia.get_info_for_feature(self.sourceset_to_test, info["featureIndex"], filter=True))
            except Exception as e:
                logger.warning("Could not fetch info for feature %s: %s", info["featureIndex"], str(e))
            feature_infos_json.append(feature_info)

        json.dump(feature_infos_json, open(f"caches/{self.model_name}/{self.sourceset_to_test}/diffs.json", "w"), indent=4)
